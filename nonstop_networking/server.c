...
void run_server(char *port)
{
    ...
    int optval = 1;
    int ret_val_1 = setsockopt(listen_sock, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
    if (ret_val_1 == -1) {
        perror("setsockopt");
        exit(1);
    }
    int ret_val_2 = setsockopt(listen_sock, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval));
    if (ret_val_2 == -1) {
        perror("setsockopt");
        exit(1);
    }
...
void close_server()
{
    ...
    fprintf(stderr, "removing files and dir...\n");
    rmdir(g_dir_name);
